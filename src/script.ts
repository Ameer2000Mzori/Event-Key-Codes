...
const keyP = document.getElementsByClassName("key-P")[0] as HTMLAnchorElement;
const numberP = document.getElementsByClassName(
  "number-P"
)[0] as HTMLAnchorElement;
const codeP = document.getElementsByClassName("code-P")[0] as HTMLAnchorElement;
...
window.addEventListener("keydown", function (event) {
  ...
  if (event.key !== undefined) {
    ...
    codeP.textContent = `${event.keyCode}`;
    handled = true;
  }
  if (handled) {
